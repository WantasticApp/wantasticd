...
import (
	...
	"wantastic-agent/internal/device/wireguard-go/tai64n"
)
...
func (msg *MessageStats) unmarshal(b []byte) error {
	...
	msg.Type = binary.LittleEndian.Uint32(b)
	msg.Receiver = binary.LittleEndian.Uint32(b[4:])
	msg.Counter = binary.LittleEndian.Uint64(b[8:])
	msg.Stats = b[16:]

	return nil
}

func (msg *MessageStats) marshal(b []byte) error {
	if len(b) < MessageTransportHeaderSize {
		return errMessageLengthMismatch
	}
	...
	binary.LittleEndian.PutUint32(b, msg.Type)
	binary.LittleEndian.PutUint32(b[4:], msg.Receiver)
	binary.LittleEndian.PutUint64(b[8:], msg.Counter)
	if len(msg.Stats) > 0 {
		copy(b[16:], msg.Stats)
	}
...
func (device *Device) ConsumeMessageResponse(msg *MessageResponse) *Peer {
	...
	ok := func() bool {
		...
		if handshake.state != handshakeInitiationCreated {
			device.log.Errorf("ConsumeMessageResponse: Invalid state %v (expected handshakeInitiationCreated)", handshake.state)
			return false
		}
		...
		if err != nil {
			device.log.Errorf("ConsumeMessageResponse: Failed to calculating shared secret (local ephemeral): %v", err)
			return false
		}
		...
		ss, err = device.staticIdentity.privateKey.sharedSecret(msg.Ephemeral)
		if err != nil {
			device.log.Errorf("ConsumeMessageResponse: Failed to calculating shared secret (static identity): %v", err)
			return false
		}
		mixKey(&chainKey, &chainKey, ss[:])
		setZero(ss[:])
		...
		if err != nil {
			device.log.Errorf("ConsumeMessageResponse: Failed to authenticate packet: %v", err)
			return false
		}
	...
	// Double-check state to avoid race condition where another thread consumed it
	if handshake.state != handshakeInitiationCreated {
		handshake.mutex.Unlock()
		device.log.Errorf("ConsumeMessageResponse: State changed during processing (race condition avoid), current: %v", handshake.state)
		return nil
	}
